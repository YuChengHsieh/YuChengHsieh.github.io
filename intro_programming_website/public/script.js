...
$(document).ready(function(){
    $('#submit_btn').click(function(){
        ...
        if($('#suggestion_column').val()===""){
            $('#reply_text').html(`<p>親!您還沒有輸入意見哦!</p>`);
            
        }
        else{
            $('#reply_text').html(`<p>我們已經收到您的訊息，謝謝您的回覆!</p>`);
    ...
    $('#login_btn').click(function(){
        user_dict.forEach(function( items,index ){
            if($('#l_account').val() === items.account){
                console.log('in');
                $('#login_name_toggle').html(`<img class="nav-link nav-link-right" style="width:50px; border-radius:100%;" src="picture/inner.png"/>
                                              <p class="nav-link nav-link-right" id="login_status">${items.name}</p>`)
                $('#login_text').html(`<a class="nav-link nav-link-right" href="#logOut_page" data-toggle="modal" data-target="#logOut_page">登出</a>`)
                alert('登入成功');
                login_clear();
                return;
            }
...
$(document).ready(function(){
    $('#logOut_btn').click(function(){
        $('#login_name_toggle').html(`<p class="nav-link nav-link-right" id="login_status">未登入</p>`);
        $('#login_text').html(`<a class="nav-link nav-link-right" href="#login_page" data-toggle="modal" data-target="#login_page">登入</a>`);
    })
})
